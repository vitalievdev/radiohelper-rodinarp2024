...
local function NewPieMenuContext(MaxPieMenuStack, MaxPieItemCount, RadiusEmpty, RadiusMin, MinItemCount, MinItemCountPerLevel)
	local obj = {
		...
		c_iRadiusEmpty = RadiusEmpty or 15,
		c_iRadiusMin = RadiusMin or 15,
...
local function BeginPieMenuEx(menuCtx)
	...
	if not isKeyJustPressed(120) then
	...
	if imgui.IsPopupOpen(pName) then
		...
		if bOpened then
			...
			if menuCtx.m_iLastFrame < (iCurrentFrame - 1) then
				if sw == 1280 and sh == 720 then
				menuCtx.m_oCenter = ImVec2(740, 543)
				end
				if sw == 1280 and sh == 800 then
				menuCtx.m_oCenter = ImVec2(740, 593)
				end
				if sw == 1280 and sh == 1024 then
				menuCtx.m_oCenter = ImVec2(740, 733)
				end
				if sw == 1366 and sh == 768 then
				menuCtx.m_oCenter = ImVec2(790, 573)
				end
				if sw == 1400 and sh == 1050 then
				menuCtx.m_oCenter = ImVec2(800, 748)
				end
				if sw == 1440 and sh == 900 then
				menuCtx.m_oCenter = ImVec2(820, 656)
				end
				if sw == 1600 and sh == 900 then
				menuCtx.m_oCenter = ImVec2(910, 656)
				end
				if sw == 1680 and sh == 1050 then
				menuCtx.m_oCenter = ImVec2(940, 748)
				end
				if sw == 1920 and sh == 1080 then
				menuCtx.m_oCenter = ImVec2(1060, 768)
				end
			end
